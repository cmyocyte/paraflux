...
import { useTraderLeaderboard, type TraderTimeframe } from "@/hooks/useLeaderboard";
import { shortenAddress, formatUsd, formatCompact, formatPercent } from "@/lib/format";
import { Button } from "@/components/ui/Button";

const TIMEFRAMES: TraderTimeframe[] = ["7d", "30d", "all"];
const PAGE_SIZE = 10;

export function TraderLeaderboard() {
  const [timeframe, setTimeframe] = useState<TraderTimeframe>("all");
  const [search, setSearch] = useState("");
  const [page, setPage] = useState(1);

  const { data } = useTraderLeaderboard(timeframe);
        ...
        <table className="w-full text-xs">
          <thead>
            <tr className="border-b border-[#21262d]">
              ...
              <th className="px-3 py-2 text-right text-[10px] font-bold uppercase tracking-wider text-zinc-500">
                PNL ({timeframe.toUpperCase()})
              </th>
              <th className="px-3 py-2 text-right text-[10px] font-bold uppercase tracking-wider text-zinc-500">
                ROI ({timeframe.toUpperCase()})
              </th>
              <th className="px-3 py-2 text-right text-[10px] font-bold uppercase tracking-wider text-zinc-500">
